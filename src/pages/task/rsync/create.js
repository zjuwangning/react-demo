...
let pathSub = null, userSub = null, createSub = null;

function RsyncTaskCreate() {
	...
	useEffect(() => {
		...
		getPath();
		getUser();
		form.setFieldValue('mode', 'MODULE')
		form.setFieldValue('nextStyle', 'days')
		setMode('MODULE')
		setRecurring('days')
	...
	const handleSubmit = values => {
		...
		for (let k in syncOtherKeyList) {
			if (values['more'].includes(syncOtherKeyList[k])) {
				temp[syncOtherKeyList[k]] = true
			}
		}
				...
				return new Promise((resolve, reject) => {
					...
					createSub = PubSub.subscribe(uuid, (_, {error})=>{
						if (error) {
							...
							notification.success({message: '创建定期同步任务成功'});
	...
	return (
		<div className={'full-page'}>
			<Row className={'title'}>创建同步任务</Row>
			<Row className={'sub-title'}>为存储池或共享文件创建定期同步任务</Row>
			<Row type={'flex'} style={{marginTop: '4vh', width: '650px'}}>
				...
				>
					...
					<Form.Item label="更多选项" name="more" valuePropName={'checked'}>
...
export default RsyncTaskCreate;
