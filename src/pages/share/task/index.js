...
function Snapshot() {
	...
	const columns = [
		...
		},
		{
			...
			width: '10%',
			render: (t, r) => r['lifetime_value']+' '+r['lifetime_unit']+'(S)'
		},
		{
			title: '时间设置',
			dataIndex: 'schedule',
		...
		},
		{
			...
			render: (t, r)=>{
				return (
					<Row type={'flex'}>
						<Button type={'link'} size={'small'} onClick={()=>{navigate('/share/snapshot-task/edit?id='+r['id'])}}>修改</Button>
	...
	return (
		<div className={'full-page'}>
			<Row className={'title'}>快照任务</Row>
			<Row className={'sub-title'}>查看，创建，删除，打开或关闭快照任务</Row>
			<Row className={'actions'}>
				<Button type={'primary'} onClick={()=>{navigate('/share/snapshot-task/create')}}>创建定期快照</Button>
			</Row>
			<Table
				size={'middle'}
				// style={{width: 1000}}
